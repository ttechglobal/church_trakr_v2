...
export default function AttendanceFlow({ church, groups, sessionsByGroup, preselectedGroupId }) {
  ...
  // Add near the top of AttendanceFlow component:
  const [awayCount, setAwayCount] = useState(0)

  useEffect(() => {
    // Fetch away count for the badge
    fetch(`/api/attendance/members?churchId=${church.id}&countAwayOnly=true`)
      .then(r => r.json())
      .then(d => setAwayCount(d.awayCount ?? 0))
      .catch(() => { })
  }, [church.id])
...
function StepGroup({ groups, sessionsByGroup, onSelect, awayCount }) {
  return (
    <div>
      ...
      {groups.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {/* Away Members entry point — always visible */}
          <a href="/away" style={{
            display: 'flex', alignItems: 'center', gap: 14,
            padding: '0.875rem 1.25rem', marginTop: 12,
            background: awayCount > 0 ? 'rgba(217,119,6,0.05)' : '#fff',
            border: `1px solid ${awayCount > 0 ? 'rgba(217,119,6,0.25)' : 'rgba(26,58,42,0.08)'}`,
            ...
            <div style={{
              ...
              background: awayCount > 0 ? 'rgba(217,119,6,0.12)' : 'rgba(26,58,42,0.06)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 18,
            }}>
              ✈️
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: 12, color: '#8a9e90', margin: 0 }}>
                {awayCount > 0
                  ? `${awayCount} member${awayCount !== 1 ? 's' : ''} currently away — not in attendance lists`
                  : 'Mark members as away so they skip attendance'
                }
              </p>
            </div>
            {awayCount > 0 && (
              <div style={{ width: 26, height: 26, borderRadius: '50%', background: '#d97706', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, flexShrink: 0 }}>
                {awayCount}
              </div>
            )}
...
function StepDate({ group, church, onBack, onSelect }) {
  ...
  const [activeQuick, setActiveQuick] = useState(null)   // 'this' | 'last'
  const [customDate, setCustomDate] = useState('')
  const [checking, setChecking] = useState(false)
  const [existsFor, setExistsFor] = useState({})     // { [date]: sessionId | false }

  // Check if a session already exists for a date
  async function checkDate(date) {
    if (existsFor[date] !== undefined) return existsFor[date]
    try {
      const res = await fetch(`/api/attendance/check?groupId=${group.id}&date=${date}`)
      const data = await res.json()
      const id = data.sessionId ?? false
      setExistsFor(prev => ({ ...prev, [date]: id }))
      return id
    } catch { return false }
  }

  // Tap quick button — highlight only, no auto-proceed
  async function tapQuick(key, date) {
    setActiveQuick(key)
    setCustomDate(date)
    // Pre-check in background so Go is instant
    checkDate(date)
  }

  // Tap Go — proceed with whatever date is selected
  ...
  const existingSessionId = existsFor[selectedDate]
  const hasExisting = existingSessionId && existingSessionId !== false
  ...
  return (
    <div>
      ...
      {/* Quick buttons — tap to SELECT, not to proceed */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
        {[
          ...
          return (
            <button key={key} onClick={() => tapQuick(key, date)}
              style={{
                padding: '1.125rem 1rem', borderRadius: 14, cursor: 'pointer', textAlign: 'left',
                border: `2px solid ${isActive ? '#1a3a2a' : 'rgba(26,58,42,0.12)'}`,
                background: isActive ? '#1a3a2a' : '#fff',
      ...
      {/* Custom date picker */}
      <div style={{ background: '#fff', border: '1px solid rgba(26,58,42,0.1)', borderRadius: 14, padding: '1rem', marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.03)' }}>
        ...
        <input type="date" value={customDate} max={toISODate(new Date())}
          ...
          style={{ width: '100%', border: '1px solid rgba(26,58,42,0.18)', borderRadius: 10, padding: '0.625rem 0.875rem', fontSize: 15, color: customDate ? '#1a2e22' : '#b0bec0', outline: 'none', minHeight: 44, background: '#fff', boxSizing: 'border-box' }}
        />
      </div>
      ...
      {selectedDate && hasExisting && (
        <div style={{
          ...
          padding: '0.875rem 1rem', marginBottom: 12,
          ...
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: '#d97706', margin: '0 0 2px' }}>
              Attendance already recorded
            </p>
            <p style={{ fontSize: 13, color: '#92400e', margin: 0, lineHeight: 1.4 }}>
              Tapping Go will open the existing session so you can edit it.
            </p>
          </div>
        </div>
      )}

      {/* Go button — always present, activates when date is selected */}
      <button
        onClick={proceed}
        disabled={!selectedDate || checking}
        style={{
          width: '100%', height: 52, borderRadius: 14, border: 'none',
          background: selectedDate && !checking
            ? (hasExisting ? 'linear-gradient(135deg,#d97706,#f59e0b)' : 'linear-gradient(135deg,#1a3a2a,#2d5a42)')
            : '#e0dbd0',
          color: selectedDate && !checking ? (hasExisting ? '#1a1a00' : '#e8d5a0') : '#8a9e90',
          fontSize: 16, fontWeight: 700,
          cursor: selectedDate && !checking ? 'pointer' : 'not-allowed',
          boxShadow: selectedDate ? '0 4px 14px rgba(26,58,42,0.25)' : 'none',
